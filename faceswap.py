...
def cpu_warning(device):
    if device == "cpu":
        print(
            "Using CPU for face enhancer. If you have a GPU, you can set device='cuda' to speed up the process. You can also set enhance=False to skip the enhancement."
        )
...
def swap_face_single(
    ...
    validate_image(target_img)
    validate_image(source_img)

    img1 = cv2.imread(target_img)
    img2 = cv2.imread(source_img)
    ...
    if plot_after:
        ...
        if enhance:
            ...
            img1_ = model_runner(img1_, model)
        # Save the image
        output_fn = os.path.join("outputs", os.path.basename(target_img))
